-- Add category expiration functionality
-- This migration adds expiration fields to categories and creates cleanup functionality

-- Add expiration fields to categories table
ALTER TABLE categories 
ADD COLUMN expires_at TIMESTAMP NULL,
ADD COLUMN auto_cleanup BOOLEAN DEFAULT false;

-- Add index for efficient expiration queries
CREATE INDEX idx_categories_expires_at ON categories(expires_at) WHERE expires_at IS NOT NULL;
...
SET expires_at = CURRENT_TIMESTAMP + INTERVAL '30 days',
    auto_cleanup = true
WHERE name = 'sources' AND is_default = true;
